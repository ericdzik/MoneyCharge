...
class _MerchantRegisterScreenState extends State<MerchantRegisterScreen> {
  ...
  static const double _fieldBorderRadius = 50.0;
  ...
  Future<void> _requestLocationPermissionAndFetch() async {
    ...
    if (kIsWeb) {
      ...
      try {
        ...
        appLogger.i('Position récupérée: Lat: ${position.latitude}, Lng: ${position.longitude}');
      ...
      try {
        appLogger.d('Plateforme mobile, demande de permission de localisation...');
        ...
        if (status.isGranted) {
          ...
          if (mounted && (status.isPermanentlyDenied || status.isRestricted)) {
            appLogger.i('Permission refusée définitivement, guidance vers les paramètres recommandée');
          }
        }
      } catch (e) {
    ...
    appLogger.d('Fin de la récupération de position. Chargement: $_isFetchingInitialLocation, Permission: $_isLocationPermissionGranted');
  }
          ...
          _currentStep < 3
              ...
              : Consumer<AuthProvider>(
                  builder: (context, authProvider, child) {
                    final bool isEnabled = !authProvider.isLoading && _acceptTerms && _isBusinessOwner;
                    return _AnimatedButton(
                      onPressed: isEnabled ? _handleRegister : null,
                      isLoading: authProvider.isLoading,
                      text: authProvider.isLoading ? 'Création...' : 'Créer mon compte',
                      backgroundColor: AppColors.orange,
                      height: 52,
                      width: 180,
                    );
      ...
      children: [
          ...
          child: ClipRRect(
            ...
            child: DropdownButtonFormField<String>(
              ...
              decoration: InputDecoration(
                ...
                prefixIcon: Icon(Icons.business_outlined, color: Colors.white.withValues(alpha: 0.5)),
              ...
              validator: (value) => value == null ? 'Veuillez sélectionner un type de commerce' : null,
            ),
          ),
        ),
        if (_selectedMerchantType == 'Autre') ...[
          ...
          _buildCustomTextField(
            ...
            validator: (value) {
              if (_selectedMerchantType == 'Autre' && (value == null || value.isEmpty)) {
    ...
    switch (_currentStep) {
      case 0:
        ...
        if (_businessNameController.text.trim().isEmpty) {
          SnackBarHelper.showError(context, 'Veuillez entrer le nom de votre business');
          return false;
        }
        ...
        if (_phoneController.text.trim().isEmpty) {
          SnackBarHelper.showError(context, 'Veuillez entrer votre numéro de téléphone');
          return false;
        }
      ...
      case 1:
        ...
        if (_selectedLocation == null) {
          SnackBarHelper.showWarning(context, 'Veuillez sélectionner un emplacement sur la carte');
          return false;
        }
        if (_selectedMerchantType == null) {
          SnackBarHelper.showError(context, 'Veuillez sélectionner un type de commerce');
          return false;
        }
        if (_selectedMerchantType == 'Autre' && _otherMerchantTypeController.text.trim().isEmpty) {
          SnackBarHelper.showError(context, 'Veuillez préciser le type de commerce');
          return false;
        }
        return true;
      ...
      case 2:
        ...
        if (!hasService) {
          SnackBarHelper.showWarning(context, 'Veuillez sélectionner au moins un service');
          return false;
        }
        if (_selectedServices['Autre'] == true && _otherServiceController.text.trim().isEmpty) {
          SnackBarHelper.showError(context, 'Veuillez préciser le service "Autre"');
          return false;
        }
        if (_openingHours.isEmpty) {
          SnackBarHelper.showWarning(context, 'Veuillez définir au moins un jour d\'ouverture');
          return false;
        }
        return true;
